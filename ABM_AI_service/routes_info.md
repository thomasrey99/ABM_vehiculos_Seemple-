...
`applied_filters` siempre se devuelve, incluso si la mayoría de los campos quedaron en `null` (el usuario puede usarlo para ver cómo interpretó la IA su consulta y corregir si hizo falta). Si la IA no logra extraer ningún filtro (o la extracción falla), la búsqueda equivale a listar todos los vehículos.
